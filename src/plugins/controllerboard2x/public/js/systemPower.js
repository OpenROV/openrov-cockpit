(function (window) {
  ...
  plugins.SystemPower.prototype.getTelemetryDefintions = function getTelemetryDefintions() {
    return [
      {
        name: 'vout',
        description: 'Voltage (mv) measured at the board'
      },
      {
        name: 'btti',
        description: 'Current (ma) measured in to the board via both battery packs'
      },
      {
        name: 'BT1I',
        description: 'Current (ma) measured from port battery tube'
      },
      {
        name: 'BT2I',
        description: 'Current (ma) measured from starboard battery tube'
      },
      {
        name: 'SC1I',
        description: 'Current (ma) measured in to port ESC'
      },
      {
        name: 'SC2I',
        description: 'Current (ma) measured in to vertical ESC'
      },
      {
        name: 'SC3I',
        description: 'Current (ma) measured in to starboard ESC'
      },
      {
        name: 'BRDI',
        description: 'Current (ma) measured at pin A0'
      },
      {
        name: 'BRDV',
        description: 'Voltage (ma) measured at pin A4'
      },
      {
        name: 'AVCC',
        description: 'Voltage reported by MCU'
      },
      {
        name: 'SC3I',
        description: 'Current (ma) measured in to starboard ESC'
      ...
      {
        name: 'ESCP',
        description: 'Power to ESCs on/off'
      },
      {
        name: 'I2CP',
        description: 'Power to external I2C bus on/off'
      }
    ];
  };
  ...
  plugins.SystemPower.prototype.listen = function listen() {
    ...
    this.cockpit.withHistory.on('status', function (status) {
      ...
      if (powerObject !== null) {
        self.cockpit.emit('plugin.controllerboard2x.systemPower.update', powerObject);
      }
    });
  };
