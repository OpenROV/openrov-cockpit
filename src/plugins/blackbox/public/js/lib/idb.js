...
function lastTelemetryItem(idb, sessionID) {
  return idb.telemetry_events.where('sessionID').equals(sessionID).last();
}
function firstTelemetryItem(idb, sessionID) {
  return idb.telemetry_events.where('sessionID').equals(sessionID).first();
}