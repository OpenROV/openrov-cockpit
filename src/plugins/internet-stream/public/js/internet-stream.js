...
(function (window, document, $) {
  ...
  InternetStream = function InternetStream(cockpit) {
    ...
    this.Plugin_Meta = {
      ...
      viewName: 'Internet Streaming plugin',
      defaultEnabled: true
    };
  ...
  InternetStream.prototype.stop = function stop() {
    ...
    this.streaming = false;
    this.cockpit.emit('internet-stream-status', {
      isStreaming: false,
      connecting: false,
  ...
  InternetStream.prototype.stream = function stream() {
    ...
    self.cockpit.emit('request_Init_Segment', function (init) {
      self.cockpit.once('local-media-init', function (initaudio) {
        socket.compress(false).emit('broadcast-stream-init', {
          video: init,
          audio: initaudio
        }, function () {
    ...
    self.cockpit.emit('internet-stream-status', {
      isStreaming: true,
      connecting: false,
  ...
  InternetStream.prototype.startService = function startService() {
    ...
    connectHandler = function () {
      ...
      self.connected = true;
      self.connecting = false;  //TODO: Add logic to restart an existing streaming session if one existed prior
    };
    var self = this;
    if (self.settings == null) {
      return;
    }
    socket = io(self.settings.streamingServerURI, {
      path: '/internetcomms',
      'multiplex': false,
