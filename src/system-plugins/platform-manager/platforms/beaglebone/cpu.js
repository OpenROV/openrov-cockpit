...
CPUInterface.prototype.LoadInfo = function (cpu) {
  ...
  return FindBeagleboneEEPROM().then(ReadEEPROM).then(ParseInfo).then(function (info) {
  ...
  function FindBeagleboneEEPROM() {
    ...
    return Promise.any([
      fs.openAsync('/sys/bus/nvmem/devices/at24-0/nvmem', 'r'),
  ...
  function ReadEEPROM(fd) {
    ...
    return readAsync(fd, new Buffer(244), 0, 244, 0).spread(function (bytesRead, buffer) {
  ...
  function ParseInfo(data) {
    ...
    return Promise.try(function () {
      ...
      console.log('CPU INFO: ' + JSON.stringify({
        'revision': revision,
        'serial': serial
      }));
      return {
...
CPUInterface.prototype.CheckSupport = function (cpu) {
  ...
  return fs.readFileAsync(path.resolve(__dirname, 'cpu/revisionInfo.json')).then(JSON.parse).then(function (json) {
