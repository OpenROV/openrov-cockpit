(function (window, $, undefined) {
  ...
  var PluginFinder = function PluginFinder(cockpit) {
    ...
    var configManager = new window.Plugins.PluginFinder.Config();  //TODO: Figure out why this blocks
  };
  ...
  PluginFinder.prototype.primeCache = function primeCache() {
    ...
    this.cockpit.rov.emit('plugin.pluginFinder.search', '', function (results) {
      ...
      results.forEach(function (plugin) {
        ...
        var giturl = plugin.url.replace('.git', '').replace('git://github.com/', 'https://api.github.com/repos/').replace('https://github.com/', 'https://api.github.com/repos/');
  ...
  PluginFinder.prototype.query = function query(q, callback) {
    callback(this.cachedAvailablePlugins.filter(function (item) {
