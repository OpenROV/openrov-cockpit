...
function capestatus(name, deps) {
  ...
  deps.app.post('/plugin/capestatus/batteries', function (req, res) {
    ...
    if (battery.name === undefined || battery.minVoltage === undefined || battery.maxVoltage === undefined) {
      res.status(400);
      res.send('Supplied battery object does not follow specification: ' + JSON.stringify(new Battery('', 0, 0)));
      return;
    }
  ...
  deps.app.delete('/plugin/capestatus/batteries', function (req, res) {
    ...
    var existing = preferences.batteries.filter(function (bat) {
        return bat.name === battery.name && parseFloat(bat.minVoltage) === parseFloat(battery.minVoltage) && parseFloat(bat.maxVoltage) === parseFloat(battery.maxVoltage);
      });
    if (existing !== undefined && existing.length === 1) {
      preferences.batteries.splice(preferences.batteries.indexOf(existing[0]), 1);
      saveConfig(deps.config);
      res.status(202).send(preferences.batteries);
    } else {
      res.status(410);
      res.send();
    }
  ...
  function handleStatus(cockpit, data) {
    if ('time' in data) {
      ...
      cockpit.emit('plugin.capestatus.time.runtime', {
        raw: data.time,
        formatted: formattedRuntime
      });
    }
    if ('vout' in data) {
      cockpit.emit('plugin.capestatus.battery.voltage', data.vout.toFixed(1));
    }
    if ('btti' in data) {
      cockpit.emit('plugin.capestatus.battery.current.out', data.btti.toFixed(3));
    }
    if ('BT1I' in data) {
      cockpit.emit('plugin.capestatus.battery.current.battery1', parseFloat(data.BT1I));
    }
    if ('BT2I' in data) {
      cockpit.emit('plugin.capestatus.battery.current.battery2', parseFloat(data.BT2I));
    }
    if ('SC1I' in data) {
      cockpit.emit('plugin.capestatus.battery.current.esc1', parseFloat(data.SC1I));
    }
    if ('SC2I' in data) {
      cockpit.emit('plugin.capestatus.battery.current.esc2', parseFloat(data.SC2I));
    }
    if ('SC3I' in data) {
      cockpit.emit('plugin.capestatus.battery.current.esc3', parseFloat(data.SC3I));
    }
    if ('cpuUsage' in data) {
      cockpit.emit('plugin.capestatus.cpu', (data.cpuUsage * 100).toFixed(0));
    }
  }
  ...
  function getPreferences(config) {
    ...
    if (preferences === undefined) {
      preferences = {
        batteries: [
          new Battery('TrustFire', 8, 13),
          new Battery('LiFePO4', 7, 10)
        ],
