(function(window, document, $) { //The function wrapper prevents leaking variables to global space
  'use strict';
  loadScript("msgpack.min.js");
  loadScript("simplepeer.min.js");
  var PeerView;
  ...
  //have loaded.
  var listening = false; //debounce guard
  PeerView.prototype.startlisten = function startlisten() {
    if ((!this.isEnabled) || (listening)) {
      return;
    }
    var self = this;
    if ((window.cockpit.rov.connection !== 'socket.io') || (window.SimplePeer == undefined) || (msgpack == undefined)) {
    ...
    socket.on('disconnect', function() {
      ...
      self.connected = false;
      self.rov.offAny(onAnyHandler);
      self.cockpit.off('x-h264-video.data', videodataHanlder);
    })
      ...
      p.on('connect', function() {
        ...
        p.on('data', function(data) {
          ...
          var cockpit_commands = [
            'plugin.rovpilot.incrementPowerLevel', 'plugin.rovpilot.setPowerLevel'
          ]
